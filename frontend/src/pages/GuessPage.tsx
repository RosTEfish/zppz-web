...
import { api, formatTime, type DesignerGuessOverview, type GuessChartRead, type GuessCommentRead } from "../api/v1";
...
const GuessChartCard = memo(function GuessChartCard({ chart, selecting, selected, candidates, canGuess, guessedUserId, guessBusy, onOpen, onGuess }: { chart: GuessChartRead; selecting: boolean; selected: boolean; candidates: DesignerGuessOverview["candidates"]; canGuess: boolean; guessedUserId?: number | null; guessBusy: boolean; onOpen: (chart: GuessChartRead) => void; onGuess: (chart: GuessChartRead, userId: number | null) => void }) {
  ...
  const emptyLabel = canGuess ? (candidates.length ? "未选择" : "暂无谱师候选") : "仅参赛者可猜";
  return (
    <Card variant="outlined" data-lane={isJ ? "j" : "normal"} data-level-slot={levelSlot ? `lv_${levelSlot}` : undefined} sx={{ position: "relative", height: "100%", display: "flex", flexDirection: "column", borderWidth: isJ ? 2 : 1, borderColor: selected ? "primary.main" : isJ ? "secondary.main" : "divider", outline: selected ? "2px solid" : "none", outlineColor: "primary.main" }}>
      ...
      <Box sx={{ p: 1.5, borderTop: 1, borderColor: "divider", bgcolor: "background.paper" }}>
        <FormControl size="small" fullWidth disabled={selecting || guessBusy || !canGuess || !candidates.length}>
...
export default function GuessPage() {
  ...
  return (
    <Stack spacing={3}>
      ...
      <GuessDetailDialog chart={active} designerGuesses={designerGuesses.data} guessedUserId={active ? guessedByChart.get(active.id) : null} guessBusy={Boolean(active && busyGuessGroup === chartGroupIdentity(active))} onGuess={saveDesignerGuess} onClose={() => setActive(null)} onChanged={async () => { const next = await charts.reload(); if (active && next) setActive(next.find((item) => item.id === active.id) || null); }} />
    </Stack>
  );
}

function GuessDetailDialog({ chart, designerGuesses, guessedUserId, guessBusy, onGuess, onClose, onChanged }: { chart: GuessChartRead | null; designerGuesses: DesignerGuessOverview | null; guessedUserId?: number | null; guessBusy: boolean; onGuess: (chart: GuessChartRead, userId: number | null) => void; onClose: () => void; onChanged: () => Promise<void> }) {
