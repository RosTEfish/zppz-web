...
import { Gauge, Music2, Settings, Users, Sparkles, CalendarClock, Archive, Vote, BarChart3, FileWarning, RadioTower } from "lucide-react";
...
const ADMIN_TABS = [
  ...
  ["draw", "曲目分配", Sparkles], ["phases", "阶段与换曲", CalendarClock], ["submissions", "投稿", Archive], ["guess", "猜谱", Vote], ["stats", "统计", BarChart3], ["banlist", "Ban 曲", FileWarning],
  ["webhooks", "Webhook", RadioTower],
] as const;
...
const AdminWebhooks = lazy(adminTabLoaders.webhooks);
...
export default function AdminPage() {
  ...
  const { isAdmin, isOwner, isPoolEditor } = useAuth();
  const navigate = useNavigate();
  const visible = isAdmin
    ? ADMIN_TABS.filter(([key]) => key !== "webhooks" || isOwner)
    : ADMIN_TABS.filter(([key]) => isPoolEditor && key === "songs");
  ...
  useEffect(() => {
    ...
    if (isAdmin && !isOwner && tab === "webhooks") navigate("/admin/overview", { replace: true });
  }, [isAdmin, isOwner, isPoolEditor, navigate, tab]);
