...
export default function AdminSubmissions() {
  ...
  const files = useApiResource(queryKeys.submissions.admin(filter, PAGE_SIZE, offset), () => api.adminSubmissions(filter, PAGE_SIZE, offset));
  ...
  return <Stack spacing={2}>
    <Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ justifyContent: "space-between" }}><Tabs value={filter} onChange={(_, value) => { setFilter(value); setSelected(new Set()); setPage(0); }}><Tab value="all" label="全部" disabled={downloading} /><Tab value="normal" label="普通" disabled={downloading} /><Tab value="j" label="J 赛道" disabled={downloading} /><Tab value="exhibition" label="场外" disabled={downloading} /></Tabs><Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: "wrap" }}><Button variant="outlined" startIcon={<Check size={16} />} disabled={downloading} onClick={() => setSelected(new Set(files.data?.items.map((item) => item.id) || []))}>全选</Button><Button variant="contained" startIcon={<Download size={16} />} disabled={!selected.size || downloading} onClick={() => void downloadSelected()}>{downloading ? "正在准备…" : `下载 ${selected.size} 份`}</Button><Button color="error" variant="outlined" startIcon={<Trash2 size={16} />} disabled={!selected.size || downloading} onClick={() => setDeleteOpen(true)}>删除 {selected.size} 份</Button></Stack></Stack>
