...
export default function AdminPhasesAndSwap() {
  ...
  return <Stack spacing={2}>
    <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 } }}>
      <Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ justifyContent: "space-between", alignItems: { md: "center" } }}><Box><Typography variant="h3">赛事阶段</Typography><Typography variant="body2" color="text.secondary">UTC 存储，以下输入与展示使用北京时间；阶段不能重叠。</Typography></Box><Stack direction="row" spacing={1}><Button variant={form.phase_mode === "auto" ? "contained" : "outlined"} disabled={busy} onClick={() => void save({ ...form, phase_mode: "auto", manual_phase: null })}>恢复自动</Button><FormControl size="small" sx={{ minWidth: 150 }}><InputLabel>手动阶段</InputLabel><Select label="手动阶段" value={form.phase_mode === "manual" ? form.manual_phase || "" : ""} onChange={(event) => setConfirmPhase(event.target.value as EventPhaseName)}>{PHASE_ORDER.map((phase) => <MenuItem key={phase} value={phase}>{PHASE_LABELS[phase]}</MenuItem>)}</Select></FormControl></Stack></Stack>
