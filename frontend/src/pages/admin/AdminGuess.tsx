...
function AuthorCandidatesEditor({ resource, setError }: { resource: ApiResource<AuthorCandidateAdmin[]>; setError: (value: string) => void }) {
  ...
  return <Paper variant="outlined" sx={{ p: 2 }}><Stack direction="row" sx={{ mb: 1.5, justifyContent: "space-between", alignItems: "center" }}><Typography variant="h3">谱师候选展示 ID</Typography><Button variant="outlined" size="small" startIcon={<SaveIcon size={15} />} onClick={() => void save()}>保存展示 ID</Button></Stack><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, 1fr)" }, gap: 1 }}>{rows.map((row, index) => <Paper key={row.user.id} variant="outlined" sx={{ p: 1.25 }}><Stack direction="row" spacing={1} sx={{ alignItems: "center" }}><Box sx={{ flex: 1, minWidth: 0 }}><Typography variant="body2" sx={{ fontWeight: 650 }}>{row.user.user_code}</Typography><Typography variant="caption" color="text.secondary">{row.user.identity === "participant" ? "参赛者" : "观众"} · {row.song_count} 首曲目</Typography></Box><TextField size="small" label="展示 ID" value={row.display_id} onChange={(event) => setRows((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, display_id: event.target.value } : item))} sx={{ width: 150 }} /></Stack></Paper>)}</Box></Paper>;
}
