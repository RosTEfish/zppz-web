...
import { Alert, Box, Checkbox, Chip, FormControlLabel, MenuItem, Paper, Select, Stack, Switch, TextField, Tooltip } from "@mui/material";
...
export default function AdminUsers() {
  ...
  const columns = useMemo<GridColDef<UserRead>[]>(() => [
    ...
    { field: "identity", headerName: "身份", width: 120, renderCell: ({ row }) => <Select size="small" value={row.identity} inputProps={{ "aria-label": `身份 ${row.user_code}` }} onChange={(event) => void change(row, { identity: event.target.value })}><MenuItem value="participant">参赛者</MenuItem><MenuItem value="audience">观众</MenuItem><MenuItem value="guest">访客</MenuItem></Select> },
